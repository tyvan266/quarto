...
To help spot-check the results (particularly for the rolling 30-day look-back averages), use the general `map` to get list of values meeting the condition at *each visit* (list of lists!). For `Z_recent`, you may want to validate that the 10-day look-back is working as expected. 
